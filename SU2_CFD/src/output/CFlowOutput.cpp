...
///////////////////////////////////////// Aggiunto Pezzo ///////////////////////////////////////
  AddHistoryOutput("PRESSURE_DEVIATION", "Std_TotalPress", ScreenOutputFormat::SCIENTIFIC, "FLOW_COEFF", "Std total pressure on all markers set in MARKER_ANALYZE", HistoryFieldType::COEFFICIENT); // GGPV
  
  AddHistoryOutput("UNCERTAINTY_QUANTIFICATION", "UQ", ScreenOutputFormat::SCIENTIFIC, "FLOW_COEFF", "UQ on all markers set in MARKER_ANALYZE", HistoryFieldType::COEFFICIENT); // GGPV
////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////// Aggiunto Pezzo /////////////////////////////////////////
  AddHistoryOutputPerSurface("PRESSURE_DEVIATION",       "Std_TotalPress",            ScreenOutputFormat::SCIENTIFIC, "FLOW_COEFF_SURF", Marker_Analyze, HistoryFieldType::COEFFICIENT); // GGPV
  
  AddHistoryOutputPerSurface("UNCERTAINTY_QUANTIFICATION",         "UQ",          ScreenOutputFormat::SCIENTIFIC, "FLOW_COEFF_SURF", Marker_Analyze, HistoryFieldType::COEFFICIENT); // GGPV 
////////////////////////////////////////////////////////////////////////////////////////////////
  if (config->GetKind_Species_Model() != SPECIES_MODEL::NONE) {
    /// DESCRIPTION: Average Species
    for (unsigned short iVar = 0; iVar < config->GetnSpecies(); iVar++) {
...
void CFlowOutput::SetAnalyzeSurface(const CSolver* const*solver, const CGeometry *geometry, CConfig *config, bool output){
  ...
  vector<su2double> Surface_TotalPressure_Marker (nMarker,0.0); //GGPV
  //vector<su2double> Surface_UQ_Marker (nMarker,0.0); //GGPV
  ...
  su2double  Tot_Surface_TotalPressure_Std = 0.0; // GGPV
  su2double  Tot_Surface_UQ_number         = 0.0; // GGPV
  ...
  for (iMarker = 0; iMarker < nMarker; iMarker++) {
    ...
    if (config->GetMarker_All_Analyze(iMarker) == YES) {
      ...
      const auto N = geometry->nVertex[iMarker]; // GGPV
      vector<su2double> Surface_TotalPressure_Vertex (N, 0.0); // GGPV
      vector<su2double> Point_Area (N, 0.0); // GGPV
      ...
      for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
        ...
        if (geometry->nodes->GetDomain(iPoint)) {
          ...
          Surface_TotalPressure_Vertex[iVertex] += TotalPressure; // GGPV
      ...
      su2double Surface_TotalPressure_Mean = Surface_TotalPressure[iMarker]; // GGPV
      
      Surface_TotalPressure_Mean /= Surface_Area[iMarker] ; // GGPV
      
      for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {// GGPV          
        Surface_TotalPressure_Marker[iMarker] += pow((Surface_TotalPressure_Vertex[iVertex] - Surface_TotalPressure_Mean), 2)*Point_Area[iVertex];              
      }
      
      Surface_TotalPressure_Marker[iMarker] = pow(Surface_TotalPressure_Marker[iMarker]/Surface_Area[iMarker], 0.5);// GGPV
    }
  }
  ...
  vector<su2double> Surface_TotalPressure_Local_Std (nMarker_Analyze,0.0); // GGPV
  vector<su2double> Surface_UQ_number_Local         (nMarker_Analyze,0.0); // GGPV
  ...
  vector<su2double> Surface_TotalPressure_Total_Std (nMarker_Analyze,0.0); // GGPV
  vector<su2double> Surface_UQ_number_Total         (nMarker_Analyze,0.0); // GGPV
    ...
    if (config->GetMarker_All_Analyze(iMarker) == YES)  {
      ...
      for (iMarker_Analyze= 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
        ...
        if (config->GetMarker_All_TagBound(iMarker) == config->GetMarker_Analyze_TagBound(iMarker_Analyze)) {
          ...
          Surface_TotalPressure_Local_Std[iMarker_Analyze] += 100000 -  Surface_TotalPressure_Marker[iMarker]; //GGPV
          Surface_UQ_number_Local[iMarker_Analyze]         += Surface_TotalPressure[iMarker]; //GGPV
  ...
  Allreduce(Surface_TotalPressure_Local_Std, Surface_TotalPressure_Total_Std);// GGPV
  Allreduce(Surface_UQ_number_Local, Surface_UQ_number_Total); // GGPV
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    if (Weight != 0.0) {
      ...
      Surface_TotalPressure_Total_Std[iMarker_Analyze]= 0.0; // GGPV
      Surface_UQ_number_Total[iMarker_Analyze]        = 0.0; // GGPV
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    su2double TotalPressure_Std = Surface_TotalPressure_Total_Std[iMarker_Analyze] * config->GetPressure_Ref();
    SetHistoryOutputPerSurfaceValue("PRESSURE_DEVIATION", TotalPressure_Std, iMarker_Analyze);
    Tot_Surface_TotalPressure_Std += TotalPressure_Std;
    config->SetSurface_TotalPressure_Std(iMarker_Analyze, TotalPressure_Std); // GGPV
    
    su2double UQ_number         = Surface_UQ_number_Total[iMarker_Analyze] * config->GetPressure_Ref();
    SetHistoryOutputPerSurfaceValue("UNCERTAINTY_QUANTIFICATION", UQ_number, iMarker_Analyze);
    Tot_Surface_UQ_number += UQ_number;
    config->SetSurface_UQ_number(iMarker_Analyze, UQ_number); // GGPV
  ...
  SetHistoryOutputValue("PRESSURE_DEVIATION", Tot_Surface_TotalPressure_Std); // GGPV
  SetHistoryOutputValue("UNCERTAINTY_QUANTIFICATION", Tot_Surface_UQ_number); // GGPV

...
def obj_f(dvs,config,state=None):
    ...
    for i_obj,this_obj in enumerate(objectives):
        ...
        if def_objs[this_obj]['OBJTYPE']=='DEFAULT':
            func += su2func(this_obj,config,state) * sign * scale  #GGPV tolto un *global_factor
...
def con_cieq(dvs,config,state=None):
    ...
    for i_obj,this_con in enumerate(constraints):
        ...
        if os.path.isfile('DIRECT/1c/history_direct.csv'):
            tempname  = 'DIRECT/1c/history_direct.csv'
            rslt_1c   = su2io.tools.read_aerodynamics('DIRECT/1c/history_direct.csv')
            val.append(rslt_1c['SURFACE_TOTAL_PRESSURE'])
        else:
            val.append(10000)
            
        if os.path.isfile('DIRECT/2c/history_direct.csv'):
            tempname  = 'DIRECT/2c/history_direct.csv'
            rslt_2c   = su2io.tools.read_aerodynamics('DIRECT/2c/history_direct.csv')
            val.append(rslt_2c['SURFACE_TOTAL_PRESSURE'])
        else:
            val.append(10000)
            
        if os.path.isfile('DIRECT/3c/history_direct.csv'):
            tempname  = 'DIRECT/1c/history_direct.csv'
            rslt_3c   = su2io.tools.read_aerodynamics('DIRECT/3c/history_direct.csv')
            val.append(rslt_3c['SURFACE_TOTAL_PRESSURE'])
        else:
            val.append(10000)
            
        if os.path.isfile('DIRECT/p1c1/history_direct.csv'):
            tempname  = 'DIRECT/1c/history_direct.csv'
            rslt_p1c1   = su2io.tools.read_aerodynamics('DIRECT/p1c1/history_direct.csv')
            val.append(rslt_p1c1['SURFACE_TOTAL_PRESSURE'])
        else:
            val.append(10000)
            
        if os.path.isfile('DIRECT/p1c2/history_direct.csv'):
            tempname  = 'DIRECT/1c/history_direct.csv'
            rslt_p1c2   = su2io.tools.read_aerodynamics('DIRECT/p1c2/history_direct.csv')
            val.append(rslt_p1c2['SURFACE_TOTAL_PRESSURE'])
        else:
            val.append(0)                              

...
void CConfig::SetPointersNull(void) {
  ...
  /*--- Actuator Disk Boundary Condition settings ---*/      //GGPV
  ...
  Surface_NormalVelocity   = nullptr;   Surface_TotalTemperature = nullptr;  Surface_TotalPressure    = nullptr;   Surface_PressureDrop    = nullptr; Surface_TotalPressure_Std = nullptr; Surface_UQ_number = nullptr;
...
void CConfig::SetMarkers(SU2_COMPONENT val_software) {
  ...
  Surface_TotalPressure_Std = new su2double[nMarker_Analyze] (); // GGPV
  Surface_UQ_number = new su2double[nMarker_Analyze] ();
  ...
  if (((val_software == SU2_COMPONENT::SU2_CFD) && ( ContinuousAdjoint || DiscreteAdjoint)) || (val_software == SU2_COMPONENT::SU2_DOT)) {
    ...
    if (nObj==1) {
      switch (Kind_ObjFunc[0]) {
        ...
        case PRESSURE_DEVIATION:         cout << "Std total pressure objective function." << endl; break; //GPPV
        case UNCERTAINTY_QUANTIFICATION: cout << "UQ number objective function." << endl; break; //GPPV
...
CConfig::~CConfig() {
  ...
  delete[] Surface_TotalPressure_Std; //GGPV
  delete[] Surface_UQ_number; //GGPV
  ...
  if (ContinuousAdjoint || DiscreteAdjoint) {
    ...
    if (nObj==1) {
      switch (Kind_ObjFunc[0]) {
        ...
        case PRESSURE_DEVIATION:          AdjExt = "_p_std";    break;// GGPV
        case UNCERTAINTY_QUANTIFICATION:  AdjExt = "_uq";       break;// GGPV
      }
    }
    else{
      AdjExt = "_combo";
